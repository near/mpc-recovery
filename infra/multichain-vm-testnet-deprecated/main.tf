...
module "gce-container" {
  ...
  container = {
    image = "us-east1-docker.pkg.dev/pagoda-discovery-platform-prod/multichain/multichain-testnet:latest"
    args  = ["start"]
    port  = "3000"
...
module "mig_template" {
  ...
  name_prefix          = "multichain-testenet-${count.index}"
...
resource "google_compute_backend_service" "multichain_backend" {
  name                  = "multichain-testnet-service"
