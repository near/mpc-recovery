...
impl Service<Uri> for UnixConnector {
    ...
    fn call(&mut self, req: Uri) -> Self::Future {
        debug!("unix connector service call start");
        let fut = async move {
            debug!("unix connector service fut start");
            let path = parse_socket_path(req)?;
            debug!("unix connector service path parsed: {}", path.display());
            debug!("unix connector service connecting via unix socket...");
