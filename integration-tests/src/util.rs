...
pub async fn spawn_mpc(release: bool, node: &str, cli: Cli) -> anyhow::Result<NodeProcess> {
    ...
    if !prebuild_mpc(release).await?.success() {
...
pub async fn prebuild_mpc(release: bool) -> anyhow::Result<ExitStatus> {
