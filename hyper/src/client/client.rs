    ...
    B::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    pub fn request(&self, mut req: Request<B>) -> ResponseFuture {
        ...
        debug!("hyper request is_http_connect: {}", is_http_connect);
        match req.version() {
            Version::HTTP_11 => (),
            Version::HTTP_10 => {
                if is_http_connect {
        ...
        debug!("hyper request version checked");

        debug!("hyper request extracting domain");
        ...
        debug!("hyper request domain extracted: {:?}", pool_key);
    ...
    async fn retryably_send_request(
        ...
        loop {
            debug!("hyper retryably_send_request before send request");
            req = match self.send_request(req, pool_key.clone()).await {
                Ok(resp) => {
                    debug!("hyper retryably_send_request ok: {:?}", resp);
                    return Ok(resp);
                }
                Err(ClientError::Normal(err)) => {
                    debug!("hyper retryably_send_request error: {:?}", err);
                    return Err(err);
                }
