...
impl UnixStream {
    ...
    where
        P: AsRef<Path>,
    {
        debug!("tokio unixstream connect start");
        let stream = mio::net::UnixStream::connect(path)?;
        debug!("tokio unixstream new unix stream");
        let stream = UnixStream::new(stream)?;

        debug!("tokio unixstream connect poll");
        poll_fn(|cx| stream.io.registration().poll_write_ready(cx)).await?;

        debug!("tokio unixstream connect take error");
