...
pub enum GenerationError {
    ...
    #[error("presignature message timed out")]
    Timeout,
}
...
pub struct PresignatureManager {
    ...
    /// The set of presignatures that were already requested to be generated. .
    requested: HashMap<PresignatureId, Instant>,
    me: Participant,
    threshold: usize,
    epoch: u64,
...
impl PresignatureManager {
    ...
    pub fn clear_requested(&mut self) {
        self.requested
            .retain(|_, instant| instant.elapsed() < crate::types::REQUESTED_TIMEOUT);
    }
    ...
    pub async fn get_or_generate(
        ...
        } else if self.requested.contains_key(&id)
            && self.requested.get(&id).unwrap().elapsed() > crate::types::PROTOCOL_PRESIG_TIMEOUT
        {
            tracing::info!(id, "presignature message timed out");
            Err(GenerationError::Timeout)
        } else {
            match self.generators.entry(id) {
                Entry::Vacant(entry) => {
                    tracing::info!(id, "joining protocol to generate a new presignature");
                    self.requested.entry(id).or_insert_with(Instant::now);
